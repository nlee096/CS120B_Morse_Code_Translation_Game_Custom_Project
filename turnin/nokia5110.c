 ...
 *
 * Link to source : https://github.com/LittleBuster/avr-nokia5110
 *
 */
...
*/		
	...
	for (y = 0; y < 7*scale; y++)
		nokia_lcd_set_pixel(nokia_lcd.cursor_x, nokia_lcd.cursor_y+y, 0);

	nokia_lcd.cursor_x += 1;
...
void nokia_lcd_inverse(char code, uint8_t scale)
{
	register uint8_t x, y;
	...
	nokia_lcd.cursor_x += 5*scale;
	for (y = 0; y < 7*scale; y++)
		nokia_lcd_set_pixel(nokia_lcd.cursor_x, nokia_lcd.cursor_y+y, 1);
...
void nokia_lcd_inverse_string(const char *str, uint8_t scale)
{
	while(*str)
		nokia_lcd_inverse(*str++, scale);
}
